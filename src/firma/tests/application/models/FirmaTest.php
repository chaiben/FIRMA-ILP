...
class FirmaTest extends Zend_Test_PHPUnit_ControllerTestCase
{
    ...
    public function testLoad(){
        ...
        $firma = $firma_mapper->load('FiDOC', 1);
        $this->assertSame('FiDOC',               $firma->fi_doc,             '"fi_doc" was not load correctly'            );
        $this->assertSame('1',                   $firma->ilp_id,             '"ilp_id" was not load correctly'            );
        $this->assertSame('FiName',              $firma->fi_name,            '"fi_name" was not load correctly'           );
        $this->assertSame('FiFirstSurname',      $firma->fi_first_surname,   '"fi_first_surname" was not load correctly'  );
        $this->assertSame('FiSecondSurname',     $firma->fi_second_surname,  '"fi_second_surname" was not load correctly' );
        $this->assertSame('2015-05-20',          $firma->fi_birthday,        '"fi_birthday" was not load correctly'       );
        $this->assertSame('2015-05-24 09:00:00', $firma->fi_timestamp,       '"fi_timestamp" was not load correctly'      );

        $firma = $firma_mapper->load('WrongDoc', 2);
        $this->assertSame(false, $firma);
    }
    ...
    public function testSave(){
        ...
        $firma = $firma_mapper->load('FiDOC', 1);
        $firma->fi_doc = 'FiDOC2';
        $firma_mapper->save($firma, 'FiDOC', 1);
        $firma = $firma_mapper->load('FiDOC2', 1);
        $this->assertSame('FiDOC2', $firma->fi_doc, '"fi_doc" was not saved correctly');
        $firma->fi_doc = 'FiDOC';
        $firma_mapper->save($firma, 'FiDOC2', 1);
    }

    public function testloadFromXades(){
        ...
        $firma->loadFromXades($xades);

        $this->assertSame('Marçal',              $firma->fi_name,            '"fi_name" was not load correctly'           );
        $this->assertSame('2',                   $firma->ilp_id,             '"ilp_id" was not load correctly'            );
        $this->assertSame('2000-01-01',          $firma->fi_birthday,        '"fi_birthday" was not load correctly'       );
    }
}
